...
Процедура УстановитьАктивныеПлагины(ПараметрыКоманды) Экспорт
	...
	Если НЕ УстановленныеПлагины["patch-compatibility"] = Неопределено Тогда
		МассивЗагруженныхПлагинов.Добавить("patch-compatibility");
	КонецЕсли;
...
Функция ЗагрузитьУстановленныеПлагины() Экспорт
	...
	ПатчСовместимости = СоответствиеНайденныхПлагинов[ИмяПлагинаПатчСовместимости];

	Если НЕ ПатчСовместимости = Неопределено Тогда
		
		ПутьКНайденномуПлагину = ПатчСовместимости;
		ИмяПлагина = ИмяПлагинаПатчСовместимости;

		Попытка
			ЗагруженныйПлагин = ЗагрузитьСценарий(ПутьКНайденномуПлагину); //: ИнформацияОСценарии
			УстановитьПлагин(ИмяПлагина, ЗагруженныйПлагин);
		Исключение
			Лог.КритичнаяОшибка("Не удалось загрузить плагин <%1> по причине: %2", "Патч совместимости", ОписаниеОшибки());
		КонецПопытки;
