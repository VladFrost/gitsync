...
КаталогCoverage = ОбъединитьПути(КаталогПроекта, "coverage");


ФС.ОбеспечитьПустойКаталог(КаталогCoverage);
ПутьКСтат = ОбъединитьПути(КаталогПроекта, "stat.json");
...
Команда.ДобавитьПараметр(""""+ОбъединитьПути(ТекущийСценарий().Каталог, "test.os")+"""");    
